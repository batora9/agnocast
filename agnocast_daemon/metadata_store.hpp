...
public:
  ...
  std::unordered_map<std::string, std::unique_ptr<TopicWrapper>> topic_map_;
  ...
  TopicWrapper * find_or_create_topic(const std::string & topic_name);

  // Caller must hold global_mutex_ (at least shared_lock) before calling.
  TopicWrapper * find_topic(const std::string & topic_name) const;
